...
class TestTaskNameValidator(unittest.TestCase):
    ...
    def test_sanitize_control_characters(self):
        ...
        for char in result:
            self.assertFalse(ord(char) < 32 or ord(char) == 127, 
    ...
    def test_github_issue_names(self):
        ...
        for issue_name in github_issues:
            with self.subTest(issue_name=issue_name):
                ...
                # Should be readable
                self.assertIn('Fix', result.replace('-', ' ') or 
                             'Feature' in result.replace('-', ' ') or
                             'Bug' in result.replace('-', ' ') or
                             'Enhancement' in result.replace('-', ' '))
