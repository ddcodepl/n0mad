...
from core.managers.notification_manager import EventType, NotificationEvent, NotificationManager
from core.services.slack_message_builder import MessageTemplate, SlackMessageBuilder, TaskStatusChangeData
from core.services.slack_service import SlackApiClient, SlackMessage, SlackMessageResult

# Import modules to test
from utils.slack_config import MessagePriority, SlackConfig, SlackConfigManager
from utils.slack_security import DataSanitizer, InputValidator, SensitiveDataType, SlackAuditLogger, SlackSecurityManager
...
class TestSlackConfig:
    ...
    def test_slack_config_creation(self):
        ...
        config = SlackConfig(bot_token="xoxb-test-token", default_channel="#general", enabled=True)
    ...
    def test_slack_config_validation(self):
        ...
        config = SlackConfig(bot_token="xoxb-test-token", rate_limit_per_minute=60, retry_attempts=3)
    ...
    def test_channel_priority_routing(self):
        ...
        config = SlackConfig(bot_token="xoxb-test-token", default_channel="#general", error_channel="#errors")
...
class TestSlackAuditLogger:
    ...
    def test_log_successful_interaction(self):
        ...
        self.audit_logger.log_slack_interaction(action="send_message", channel="#general", success=True, message_id="1234567890.123456")
...
class TestSlackMessageBuilder:
    ...
    def test_status_change_message(self):
        ...
        message = self.message_builder.build_task_status_change_message(data=data, channel="#general", priority=MessagePriority.MEDIUM)
    ...
    def test_completion_message(self):
        ...
        message = self.message_builder.build_task_completion_message(data=data, channel="#general")
    ...
    def test_failure_message(self):
        ...
        message = self.message_builder.build_task_failure_message(data=data, channel="#errors", error_details="Connection timeout")
    ...
    def test_priority_determination(self):
        ...
        priority = self.message_builder.get_priority_for_status_change("in-progress", "done")
        assert priority == MessagePriority.HIGH

        # Failure should be urgent priority
        priority = self.message_builder.get_priority_for_status_change("in-progress", "failed")
        assert priority == MessagePriority.URGENT

        # Regular transitions should be medium priority
        priority = self.message_builder.get_priority_for_status_change("pending", "in-progress")
...
class TestSlackApiClient:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        self.config = SlackConfig(bot_token="xoxb-test-token", default_channel="#general")
    ...
    def test_successful_message_send(self, mock_webclient):
        ...
        client = SlackApiClient(self.config)
        message = SlackMessage(channel="#general", text="Test message", priority=MessagePriority.MEDIUM)
    ...
    def test_failed_message_send(self, mock_webclient):
        ...
        client = SlackApiClient(self.config)
        message = SlackMessage(channel="#nonexistent", text="Test message", priority=MessagePriority.MEDIUM)
...
class TestNotificationManager:
    ...
    def test_event_filtering(self):
        ...
        self.notification_manager.configure_filter(ignored_statuses={"pending"})
...
class TestSlackSecurityManager:
    ...
    def test_secure_notification_data(self):
        ...
        data = {
            ...
            "from_status": "pending",
            "to_status": "in-progress",
            "metadata": {"password": "secret123", "description": "Contact admin@example.com"},
        }
...
class TestIntegrationWorkflow:
    ...
    def test_error_handling_and_recovery(self):
        ...
        try:
            notification_manager.notify_status_change(task_id=None, task_title="", from_status="", to_status="")  # Invalid data
