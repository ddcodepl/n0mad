...
def test_exponential_backoff_strategy():
    ...
    strategy = ExponentialBackoffStrategy(base_interval_minutes=2, max_interval_minutes=32, backoff_multiplier=2.0)
    ...
    # Test configuration
    strategy.configure({"base_interval_minutes": 1, "max_interval_minutes": 16, "backoff_multiplier": 1.5})
    config = strategy.get_configuration()
...
def test_adaptive_strategy():
    ...
    strategy = AdaptiveStrategy(base_interval_minutes=5, min_interval_minutes=1, max_interval_minutes=30, queue_threshold=5)
...
def test_strategy_factory():
    ...
    fixed_strategy = PollingStrategyFactory.create_strategy(PollingStrategyType.FIXED_INTERVAL, {"interval_minutes": 3})
    assert isinstance(fixed_strategy, FixedIntervalStrategy)
    assert fixed_strategy.get_configuration()["interval_minutes"] == 3

    adaptive_strategy = PollingStrategyFactory.create_strategy(PollingStrategyType.ADAPTIVE, {"base_interval_minutes": 10})
...
def performance_benchmark():
    ...
    context = PollingContext(consecutive_failures=2, queue_depth=10, system_load=0.7, error_rate=0.05)
    ...
    for strategy in strategies:
        ...
        print(f"   📊 {strategy.get_strategy_type().value}: " f"{ops_per_second:.1f} decisions/sec ({duration:.4f}s total)")
...
def main():
    ...
    try:
        ...
        for strategy_type in PollingStrategyFactory.get_available_strategies():
            strategy = PollingStrategyFactory.create_strategy(strategy_type, DEFAULT_STRATEGY_CONFIGS.get(strategy_type, {}))
