...
from core.services.slack_service import SlackApiClient, SlackMessage
...
class NotificationManager:
    ...
    def _should_process_event(self, event: NotificationEvent) -> bool:
        ...
        if event.event_type == EventType.TASK_STATUS_CHANGE:
            ...
            if (to_status in self._filter.ignored_statuses or 
                from_status in self._filter.ignored_statuses):
                return False
            
            # Check enabled statuses (if specified)
            if (self._filter.enabled_statuses and 
                to_status not in self._filter.enabled_statuses):
                return False
            ...
            if (self._filter.enabled_transitions and 
                transition not in self._filter.enabled_transitions):
                return False
    ...
    def _handle_status_change_event(self, event: NotificationEvent):
        ...
        if self._slack_client and self._slack_message_builder:
            try:
                ...
                message_config = self._slack_message_builder.build_task_status_change_message(
                    data=status_data,
                    channel=channel,
