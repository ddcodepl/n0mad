...
from src.clients.claude_engine_invoker import ClaudeEngineInvoker, InvocationResult
from src.clients.improved_notion_wrapper import ImprovedNotionWrapper
...
from src.core.managers.task_file_manager import CopyResult, TaskFileManager
...
from src.core.processors.multi_queue_processor import MultiQueueProcessor
from src.core.processors.multi_status_processor import MultiStatusProcessor
from src.core.processors.prepare_mode_processor import PrepareTasksProcessor
...
from src.utils.performance_integration import PerformanceContext, initialize_performance_monitoring, integrate_all_components, log_performance_summary
from src.utils.polling_scheduler import CircuitBreakerConfig, PollingScheduler
from src.utils.simple_profiler import profile_context, simple_profiler

# Import configuration management
from src.utils.singleton_config import get_config, initialize_config

setup_logging(level=logging.INFO, enable_file_logging=True, use_colors=True)
...
class NotionDeveloper:
    def __init__(self, mode="refine"):
        ...
        try:
            ...
            self.global_config = initialize_config(strict_validation=True)
            ...
            # Use improved clients with better error handling
            try:
                self.notion_client = ImprovedNotionWrapper(use_async=False)
            except ConfigurationError:
                logger.warning("⚠️ Falling back to legacy NotionClientWrapper")
            ...
            while self.running:
                try:
                    ...
                    if tasks:
                        ...
                        logger.info(
                            f"Processing {len(tasks)} tasks with {max_concurrent_tasks} concurrent workers (max configured: {self.max_concurrent_tasks})"
                        )

                        with concurrent.futures.ThreadPoolExecutor(max_workers=max_concurrent_tasks) as executor:
                            # Submit all tasks for concurrent processing
                            future_to_task = {executor.submit(self.process_task_wrapper, task): task for task in tasks}
    ...
    def run_prepare_mode(self):
        """Run the prepare mode using improved PrepareTasksProcessor."""
        logger.info("🚀 Starting prepare mode - NotionTaskMasterWorkflow")

        with profile_context("prepare_mode_session"):
            try:
                # Use the new PrepareTasksProcessor for better separation of concerns
                processor = PrepareTasksProcessor(
                ...
                workflow_results = processor.process_prepare_tasks()

                # Log performance summary
                summary = simple_profiler.get_metrics_summary()
                logger.info(f"📊 Performance Summary: {summary['total_operations']} operations completed")

                return workflow_results

            except Exception as e:
                logger.error(f"❌ Prepare mode failed with error: {e}")
                return {
                    "step_results": {},
    ...
    def log_final_statistics(self):
        ...
        logger.info(
            f"  Success rate: {self.stats['tasks_processed'] / (self.stats['tasks_processed'] + self.stats['tasks_failed']) * 100 if (self.stats['tasks_processed'] + self.stats['tasks_failed']) > 0 else 0:.2f}%"
        )
        logger.info("=" * 50)
...
def main():
  ...
  n0mad --refine          # Process 'To Refine' status tasks
  n0mad --prepare         # Process 'Prepare Tasks' status
  n0mad --queued          # Process 'Queued to run' status tasks
  n0mad --multi           # Multi-status mode
  n0mad --config-help     # Show configuration help
  n0mad --config-create   # Create configuration template
  n0mad --status          # Show configuration status""",
    ...
    parser.add_argument(
        "--refine",
        action="store_true",
        help="Run in refine mode (process 'To Refine' status tasks)",
    )
    parser.add_argument(
        "--prepare",
        action="store_true",
        help="Run in prepare mode (process 'Prepare Tasks' status)",
    )
    parser.add_argument(
        "--queued",
        action="store_true",
        help="Run in queued mode (process 'Queued to run' status tasks only)",
    )
    parser.add_argument(
        "--multi",
        action="store_true",
        help="Run in multi-status mode (process multiple status types)",
    )
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="N0MAD v0.0.6 - Notion Orchestrated Management & Autonomous Developer",
    )
...
def create_config_template(working_dir: Optional[str] = None):
    ...
    try:
        ...
        print("  4. Test with: n0mad --config-status")
...
def perform_health_check(working_dir: Optional[str] = None):
    ...
    try:
        ...
        health_status = {"overall_healthy": True, "checks": {}, "warnings": [], "errors": []}
        ...
        if health_status["errors"] or health_status["warnings"]:
            ...
            print("    n0mad --config-help     # Configuration assistance")
            print("    n0mad --config-create   # Create config template")
            print("    n0mad --config-status   # Check configuration")
            print()

        if health_status["overall_healthy"]:
            print("🎉 Ready to run! Try:")
            print("  n0mad --help           # Show all available commands")
            print("  n0mad --refine         # Process Notion tasks")
