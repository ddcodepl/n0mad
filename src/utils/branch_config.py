...
class BranchCreationConfig:
    ...
    def __post_init__(self):
        ...
        if self.allowed_base_branches is None:
            self.allowed_base_branches = ["master", "main", "develop", "dev"]

        if self.forbidden_branch_patterns is None:
            self.forbidden_branch_patterns = ["master", "main", "HEAD", "origin/*", "refs/*"]
...
class BranchConfigManager:
    ...
    def _load_from_environment(self) -> None:
        ...
        env_mappings = {
            ...
            f"{self.ENV_PREFIX}NAMING_STRATEGY": (
                "naming_strategy",
                lambda x: BranchNamingStrategy(x),
            ),
            f"{self.ENV_PREFIX}BRANCH_PREFIX": ("branch_prefix", str),
            f"{self.ENV_PREFIX}MAX_BRANCH_NAME_LENGTH": ("max_branch_name_length", int),
            f"{self.ENV_PREFIX}FORCE_CREATION": (
                "force_branch_creation",
                lambda x: x.lower() in ["true", "1", "yes"],
            ),
            f"{self.ENV_PREFIX}SWITCH_TO_BRANCH": (
                "switch_to_new_branch",
                lambda x: x.lower() in ["true", "1", "yes"],
            ),
            f"{self.ENV_PREFIX}PUSH_TO_REMOTE": (
                "push_to_remote",
                lambda x: x.lower() in ["true", "1", "yes"],
            ),
            f"{self.ENV_PREFIX}FAIL_ON_ERROR": (
                "fail_task_on_branch_error",
                lambda x: x.lower() in ["true", "1", "yes"],
            ),
    ...
    def _validate_configuration(self) -> None:
        ...
        if self._config.allowed_base_branches and self._config.default_base_branch not in self._config.allowed_base_branches:
    ...
    def create_default_config_file(self) -> None:
        ...
        config_content = {
            ...
            "checkbox_property_names": ["New Branch", "Create Branch", "Branch"],
            "require_explicit_checkbox": True,
            "allowed_base_branches": ["master", "main", "develop"],
            "forbidden_branch_patterns": ["master", "main", "HEAD"],
        }
