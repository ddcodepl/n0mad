...
class GlobalConfigManager:
    ...
    def _load_local_env_files(self):
        ...
        local_env_files = [self.working_dir / ".env", self.working_dir / ".env.local"]
    ...
    def _validate_configuration(self):
        ...
        if missing_required:
            # Provide helpful guidance for missing variables
            if "NOTION_TOKEN" in missing_required or "NOTION_BOARD_DB" in missing_required:
                notion_token = self.config.get("NOTION_TOKEN")
                notion_db = self.config.get("NOTION_BOARD_DB")
                self._handle_missing_notion_config(notion_token, notion_db)
        ...
        if not security_results["secure"]:
            if self.strict_validation:
                ...
                # Check if we're missing required Notion configuration and provide helpful guidance
                notion_token = self.config.get("NOTION_TOKEN")
                notion_db = self.config.get("NOTION_BOARD_DB")
    ...
    def enhanced_validation(self) -> Dict[str, Any]:
        ...
        results = {"valid": True, "errors": [], "warnings": [], "field_results": {}}
        ...
        has_api_key = any(self.config.get(key) and self.validate_api_key_format(self.config.get(key)) for key in api_key_fields)

        if not has_api_key:
            results["warnings"].append(
                "No valid AI API keys found. At least one of OPENAI_API_KEY, " "OPENROUTER_API_KEY, or ANTHROPIC_API_KEY is recommended."
            )
    ...
    def _handle_missing_notion_config(self, notion_token: Optional[str], notion_db: Optional[str]):
        """Provide helpful guidance when Notion configuration is missing."""
        logger.error("")
        logger.error("🔑 Missing required Notion configuration:")
        
        if not notion_token:
            logger.error("   ❌ NOTION_TOKEN is required but not found")
        if not notion_db:
            logger.error("   ❌ NOTION_BOARD_DB is required but not found")
        ...
        if env_file.exists():
            logger.error(f"📁 Found .env file at: {env_file}")
            logger.error("💡 Please add the missing variables to your .env file:")
            logger.error("")
            if not notion_token:
                logger.error("   NOTION_TOKEN=your_notion_integration_token_here")
            if not notion_db:
                logger.error("   NOTION_BOARD_DB=your_notion_database_id_here")
            logger.error("")
        else:
            logger.error(f"📁 No .env file found in current directory: {self.working_dir}")
            logger.error("💡 Please create a .env file with your Notion configuration:")
            logger.error("")
            logger.error("   cat > .env << EOF")
            logger.error("   NOTION_TOKEN=your_notion_integration_token_here")
            logger.error("   NOTION_BOARD_DB=your_notion_database_id_here")
            logger.error("   EOF")
            logger.error("")
        
        logger.error("🔗 To get your Notion configuration:")
        logger.error("   1. NOTION_TOKEN:")
        logger.error("      - Go to https://www.notion.so/my-integrations")
        logger.error("      - Create a new integration")
        logger.error("      - Copy the 'Internal Integration Token'")
        logger.error("")
        logger.error("   2. NOTION_BOARD_DB:")
        logger.error("      - Open your Notion database page")
        logger.error("      - Copy the database ID from the URL")
        logger.error("      - It's the 32-character string after the last '/' in the URL")
        logger.error("")
        logger.error("💡 After creating .env file, run n0mad again from this directory")
        logger.error("")
