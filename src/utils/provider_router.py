...
from src.clients.openrouter_client import OpenRouterClient, OpenRouterError
from src.utils.config import config_manager
from src.utils.model_parser import ModelParser, ParsedModel, ValidationError
from src.utils.security_validator import sanitize_log_message, security_validator
    ...
    def route_request(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"Routing decision: {route_decision.value} for model '{sanitize_log_message(model_string or 'default')}' -> {parsed_model.provider}/{parsed_model.model}"
            )

            # Route to appropriate provider
            if route_decision == RouteDecision.OPENAI_DIRECT:
                return self._route_to_openai(
                    parsed_model,
                    content,
                    system_prompt,
                    shutdown_flag,
                ...
                return self._route_to_fallback(
                    parsed_model,
                    content,
                    system_prompt,
                    shutdown_flag,
