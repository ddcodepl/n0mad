...
from core.processors.branch_integrated_processor import BranchIntegratedProcessor, BranchIntegratedTaskItem, BranchProcessingResult
...
class TestBranchIntegration(unittest.TestCase):
    ...
    def create_test_task_item(self, task_id: str = "TEST-001", title: str = "Test Task", branch_requested: bool = False) -> BranchIntegratedTaskItem:
        ...
        return BranchIntegratedTaskItem(
            ...
            metadata={"taskmaster_task": {"properties": {"New Branch": {"type": "checkbox", "checkbox": branch_requested}}}},
        )
    ...
    def test_enhance_tasks_with_branch_metadata(self):
        ...
        standard_tasks = [
            QueuedTaskItem(
                ...
                queued_time=datetime.now(),
                metadata={"taskmaster_task": {"properties": {"New Branch": {"type": "checkbox", "checkbox": True}}}},
            ),
            QueuedTaskItem(
                task_id="TEST-002",
                page_id="page_002",
    ...
    def test_process_single_task_with_branch_success(self, mock_execute):
        ...
        mock_execute.return_value = {"status": "success", "task_processing_completed": True}
    ...
    def test_process_single_task_execution_failure(self, mock_execute):
        ...
        mock_execute.return_value = {"status": "failed", "error": "Task processing error"}
    ...
    def test_create_error_result(self):
        ...
        task_item = self.create_test_task_item(task_id="TEST-ERROR", title="Error task", branch_requested=True)
    ...
    def test_get_branch_integration_statistics(self):
        ...
        with (
            patch.object(self.processor.branch_integration_manager, "get_integration_statistics") as mock_int_stats,
            ...
            mock_feed_stats.return_value = {"total_feedback": 10, "branches_created": 4}
    ...
    def test_full_integration_workflow(self, mock_get_config):
        ...
        with patch.object(self.processor, "_discover_and_prioritize_tasks") as mock_discover:
            ...
            # Mock task execution
            with (
                patch.object(self.processor, "_execute_single_task") as mock_execute,
                patch.object(
                    self.processor.branch_integration_manager,
                    "integrate_with_multi_queue_processor",
                ) as mock_integrate,
            ):

                mock_execute.return_value = {"status": "success", "completed_successfully": True}
                ...
                self.assertTrue(result["branch_integration"]["created"])
                self.assertEqual(
                    result["branch_integration"]["branch_name"],
                    "WORKFLOW-001-integration-test-task",
                )
...
class TestBranchIntegrationEdgeCases(unittest.TestCase):
    ...
    def test_branch_integration_initialization_failure(self):
        ...
        with patch(
            "core.processors.branch_integrated_processor.get_branch_config",
    ...
    def test_processing_with_branch_integration_disabled(self):
        ...
        with patch.object(processor, "_execute_single_task") as mock_execute:
            mock_execute.return_value = {"status": "success", "completed": True}
