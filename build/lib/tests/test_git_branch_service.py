...
from core.services.branch_service import BranchCreationResult, BranchOperation, GitBranchService, TaskNameValidator
...
class TestGitBranchService(unittest.TestCase):
    ...
    def test_create_git_branch_success(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="Switched to a new branch 'feature-branch'\n", stderr="")
    ...
    def test_create_branch_for_task_complete_flow(self):
        ...
        with (
            patch.object(self.service, "_is_git_repository", return_value=True),
            ...
            operation = self.service.create_branch_for_task(task_id="TASK-123", task_title="Fix user login bug", base_branch="develop")
    ...
    def test_create_branch_for_task_not_git_repo(self):
        ...
        with patch.object(self.service, "_is_git_repository", return_value=False):
            ...
            operation = self.service.create_branch_for_task(task_id="TASK-123", task_title="Fix bug")
    ...
    def test_create_branch_for_task_invalid_base_branch(self):
        ...
        with (
            ...
            operation = self.service.create_branch_for_task(task_id="TASK-123", task_title="Fix bug", base_branch="non-existent")
    ...
    def test_create_branch_for_task_force_creation(self):
        ...
        with (
            ...
            operation = self.service.create_branch_for_task(task_id="TASK-123", task_title="Fix bug", force=True)
...
class TestGitBranchServiceIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            subprocess.run(["git", "init"], cwd=self.temp_dir, capture_output=True, check=True, timeout=10)
            ...
            subprocess.run(["git", "add", "README.md"], cwd=self.temp_dir, capture_output=True, timeout=10)
            subprocess.run(
    ...
    def test_real_branch_creation(self):
        ...
        operation = self.service.create_branch_for_task(task_id="TEST-001", task_title="Integration test branch", base_branch="master")
    ...
    def test_real_branch_already_exists(self):
        ...
        operation1 = self.service.create_branch_for_task(task_id="TEST-002", task_title="Duplicate test", base_branch="master")
        self.assertEqual(operation1.result, BranchCreationResult.SUCCESS)

        # Try to create same branch again
        operation2 = self.service.create_branch_for_task(task_id="TEST-002", task_title="Duplicate test", base_branch="master")
    ...
    def test_real_invalid_base_branch(self):
        ...
        operation = self.service.create_branch_for_task(task_id="TEST-003", task_title="Invalid base test", base_branch="non-existent-branch")
