...
from core.managers.enhanced_status_transition_manager import (
    EnhancedStatusTransitionManager,
    EnhancedStatusTransition,
    EnhancedTransitionResult
)
...
class TestEnhancedStatusTransitionManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('core.managers.enhanced_status_transition_manager.get_global_config', 
    ...
    def test_successful_enhanced_transition_with_commit(self, mock_logger):
        ...
        with patch.object(self.manager.validation_service, 'validate_task_transition', 
                         return_value=mock_validation_operation), \
             patch.object(self.manager, '_execute_base_transition', 
                         return_value=mock_base_transition), \
             patch.object(self.manager.git_commit_service, 'execute_commit', 
                         return_value=mock_commit_operation), \
             patch.object(self.manager.commit_message_generator, 'generate_commit_message', 
    ...
    def test_commit_failure_with_rollback_success(self):
        ...
        with patch.object(self.manager.validation_service, 'validate_task_transition', 
             ...
             patch.object(self.manager, '_execute_base_transition', 
                         return_value=mock_base_transition), \
             patch.object(self.manager, '_create_commit_for_transition', 
                         return_value=mock_commit_operation), \
             patch.object(self.manager, '_rollback_status_transition', 
                         return_value=True):
    ...
    def test_transition_without_commit_requirement(self):
        ...
        with patch.object(self.manager, '_execute_base_transition', 
                         return_value=mock_base_transition), \
             patch.object(self.manager, '_requires_commit', 
                         return_value=False), \
             patch.object(self.manager, 'is_valid_transition', 
                         return_value=True):
        ...
        for from_status, to_status, expected in test_cases:
            with self.subTest(from_status=from_status, to_status=to_status):
                ...
                self.assertEqual(result, expected, 
                               f"Failed for {from_status} -> {to_status}")
    ...
    def test_enhanced_transition_history_tracking(self):
        ...
        with patch.object(self.manager, '_execute_base_transition', return_value=mock_base_transition), \
            ...
            for i in range(3):
                self.manager.transition_status_enhanced(
                    page_id=f"page_{i}",
                    from_status="todo",
                    to_status="in-progress"
                )
    ...
    def test_enhanced_statistics_calculation(self):
        ...
        transitions = [
            ...
            (EnhancedTransitionResult.ROLLBACK_SUCCESS, ValidationResult.SUCCESS, CommitResult.FAILED)
        ]
    ...
    def test_rollback_status_transition_success(self):
        ...
        with patch.object(self.manager.__class__.__bases__[0], 'transition_status', 
                         return_value=mock_rollback_transition):
            result = self.manager._rollback_status_transition(
                page_id=self.test_page_id,
