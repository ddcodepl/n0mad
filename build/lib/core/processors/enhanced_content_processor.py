...
from core.managers.branch_integration_manager import BranchIntegrationManager
from core.managers.branch_feedback_manager import BranchFeedbackManager
from core.services.branch_service import CheckboxStateDetector
from utils.checkbox_utils import CheckboxUtilities
from utils.branch_config import get_branch_config
...
class EnhancedContentProcessor(ContentProcessor):
    ...
    def __init__(self, 
        ...
        if self.enable_branch_integration:
            try:
                ...
                self.branch_feedback_manager = BranchFeedbackManager(
    ...
    def process_task(self, task: Dict[str, Any], shutdown_flag: callable = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if (self.enable_branch_integration and 
                result["branch_integration"]["branch_created"] and 
                result.get("status") == "completed"):
                
                self._update_branch_with_processing_results(
                    task_id, 
            ...
            if (self.enable_branch_integration and 
                result["branch_integration"]["branch_requested"]):
                result["branch_integration"]["integration_error"] = str(e)
                self.branch_feedback_manager.report_integration_result(
                    self._create_failed_integration_operation(task_id, title, str(e))
                )
    ...
    def _handle_branch_integration(self, 
                                   task: Dict[str, Any], 
                                   page_id: str, 
        ...
        try:
            ...
            if branch_integration_op.branch_operation:
                integration_result["branch_created"] = (
                    branch_integration_op.branch_operation.result.value == "success"
                )
    ...
    def _validate_branch_creation_allowed(self, 
                                          task_id: str, 
                                          branch_preferences: Dict[str, Any]) -> bool:
        """
    ...
    def _update_branch_with_processing_results(self, 
                                               task_id: str, 
                                               branch_name: str, 
    ...
    def _create_failed_integration_operation(self, 
                                             task_id: str, 
                                             task_title: str, 
                                             error: str):
    ...
    def get_branch_integration_statistics(self) -> Dict[str, Any]:
        ...
        if not self.enable_branch_integration:
            return {
                ...
                "message": "Branch integration is disabled"
            }
    ...
    def is_branch_integration_enabled(self) -> bool:
        ...
        return (self.enable_branch_integration and 
                hasattr(self, 'branch_config') and 
