...
from src.utils.config import DEFAULT_MODEL, REFINEMENT_PROMPT
from src.utils.global_config import get_global_config
from src.utils.model_parser import ModelParser, ValidationError
...
class OpenAIClient:
            ...
            try:
                ...
                if openrouter_api_key in error_msg:
                    error_msg = error_msg.replace(
                        openrouter_api_key,
                        self.security_manager.mask_sensitive_value(openrouter_api_key),
                    )
    ...
    def get_client_statistics(self) -> Dict[str, Any]:
        ...
        if self.openrouter_client:
            return {"provider": "openrouter", "stats": self.openrouter_client.get_statistics()}
        else:
            return {
                "provider": "openai",
                "stats": {"note": "OpenAI client statistics not tracked in current implementation"},
            }
