...
from src.utils.env_security import mask_sensitive_dict
from src.utils.global_config import get_global_config
from src.utils.logging_config import get_logger, log_key_value, log_list_items, log_section_header, log_subsection_header
...
class NotionClientWrapper:
    def __init__(self, token: Optional[str] = None, database_id: Optional[str] = None, max_retries: int = 3):
        ...
        if not self.token:
            raise ValueError(
                "Notion token not found. Set NOTION_TOKEN environment variable or configure it globally. " "Run 'nomad --config-help' for setup instructions."
            )
    ...
    def test_connection(self) -> bool:
        try:
            database = self._retry_with_exponential_backoff(self.client.databases.retrieve, database_id=self.database_id)
    ...
    def create_status_filter(self, status_value: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if prop_type == "select":
                filter_dict = {"property": "Status", "select": {"equals": status_value}}
            elif prop_type == "status":
                filter_dict = {"property": "Status", "status": {"equals": status_value}}
            elif prop_type == "multi_select":
                filter_dict = {"property": "Status", "multi_select": {"contains": status_value}}
            else:
    ...
    def query_database(
        ...
        try:
            ...
            response = self._retry_with_exponential_backoff(self.client.databases.query, **query_params)
            return response
        except Exception as e:
    ...
    def query_tickets_by_status(self, status: str, include_all_pages: bool = True) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            while True:
                ...
                response = self.query_database(filter_dict=status_filter, start_cursor=start_cursor, page_size=100)
    ...
    def update_tickets_status_batch(self, page_ids: List[str], new_status: str) -> Dict[str, Any]:
        ...
        for i, page_id in enumerate(page_ids):
            try:
                ...
                error_info = {"page_id": page_id, "error": str(e), "attempted_status": new_status}
    ...
    def _upload_file_to_page_property(self, page_id: str, file_path: str, property_name: str = "Tasks") -> Dict[str, Any]:
        ...
        try:
            ...
            clear_property = {property_name: {"files": []}}

            # Clear existing files first
            self._retry_with_exponential_backoff(self.client.pages.update, page_id=page_id, properties=clear_property)
                    ...
                    "files": [
                        {
                            "name": file_name,
                            "type": "external",
                            "external": {"url": f"file://localhost/{os.path.abspath(file_path)}"},  # Local file reference
                        }
                    ]
                }
            }
            ...
            updated_page = self._retry_with_exponential_backoff(self.client.pages.update, page_id=page_id, properties=file_property)
    ...
    def _attempt_rollback(self, finalized_tickets: List[Dict[str, Any]], rollback_status: str = "Preparing Tasks") -> Dict[str, List]:
        ...
        for ticket_info in finalized_tickets:
            ...
            try:
                ...
                rollback_results["rollback_successful"].append({**ticket_info, "rollback_status": rollback_status})

                logger.info(f"✅ Successfully rolled back ticket {ticket_id} to '{rollback_status}'")

            except Exception as e:
                rollback_results["rollback_failed"].append({**ticket_info, "rollback_error": str(e)})
    ...
    def update_page(self, page_id: str, properties: Dict[str, Any]) -> Dict[str, Any]:
        try:
            updated_page = self._retry_with_exponential_backoff(self.client.pages.update, page_id=page_id, properties=properties)
    ...
    def update_page_property(self, page_id: str, property_name: str, property_value: str) -> Dict[str, Any]:
        ...
        try:
            ...
            properties = {property_name: {"rich_text": [{"type": "text", "text": {"content": property_value}}]}}
                                ...
                                async with session.delete(url, headers=headers) as retry_response:
                                    if retry_response.status == 200:
                                        ...
                                        return {
                                            "block_id": block_id,
                                            "success": False,
                                            "error": f"HTTP {retry_response.status}",
                                        }
                                ...
                                async with session.delete(url, headers=headers) as retry_response:
                                    if retry_response.status == 200:
                                        ...
                                        return {
                                            "block_id": block_id,
                                            "success": False,
                                            "error": "HTTP 409 - Conflict (dependencies)",
                                        }
                                    else:
    ...
    async def _create_blocks_async(
        self,
        page_id: str,
        blocks: List[Dict],
    ...
    async def _delete_blocks_hierarchical(
        self,
        deletion_strategy: Dict[str, List[str]],
        headers: Dict[str, str],
        shutdown_flag: callable = None,
    ):
        """
                ...
                if paragraph.strip():
                    if paragraph.startswith("# "):
                        ...
                        new_blocks.append(
                            {
                                "type": "heading_3",
                                "heading_3": {"rich_text": [{"type": "text", "text": {"content": paragraph[4:]}}]},
                            }
                        )
                    elif paragraph.startswith("- "):
                        new_blocks.append(
                            {
                                ...
                                "bulleted_list_item": {"rich_text": [{"type": "text", "text": {"content": paragraph[2:]}}]},
                            }
                        )
                    else:
                        new_blocks.append(
