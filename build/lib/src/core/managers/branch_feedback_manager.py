...
from src.core.managers.branch_integration_manager import BranchIntegrationOperation, IntegrationResult
from src.core.managers.feedback_manager import FeedbackManager, ProcessingStage
from src.core.services.branch_service import BranchCreationResult, BranchOperation
...
class BranchFeedbackManager:
    ...
    def report_checkbox_detection(
        self,
        task_id: str,
        page_id: Optional[str],
        checkbox_detected: bool,
        branch_requested: bool,
        checkbox_properties: List[str] = None,
    ) -> None:
        """
        ...
        if self.feedback_manager and page_id:
            ...
            self.feedback_manager.add_feedback(page_id=page_id, stage=stage, message=message, details=details)
    ...
    def report_branch_creation_started(
        self,
        task_id: str,
        page_id: Optional[str],
        branch_name: str,
        ...
        # Integrate with existing feedback manager
        if self.feedback_manager and page_id:
            self.feedback_manager.add_feedback(page_id=page_id, stage=ProcessingStage.PROCESSING, message=message, details=details)

        if self.enable_detailed_logging:
            logger.info(f"🌿 Creating branch '{branch_name}' from '{base_branch}' for task {task_id}")

    def report_branch_creation_result(self, task_id: str, page_id: Optional[str], branch_operation: BranchOperation) -> None:
        """
    ...
    def _report_branch_success(self, task_id: str, page_id: Optional[str], branch_operation: BranchOperation) -> None:
    ...
    def _report_branch_already_exists(self, task_id: str, page_id: Optional[str], branch_operation: BranchOperation) -> None:
    ...
    def _report_branch_failure(self, task_id: str, page_id: Optional[str], branch_operation: BranchOperation) -> None:
    ...
    def report_integration_result(self, integration_operation: BranchIntegrationOperation) -> None:
        """
        ...
        details = {
            ...
            "branch_created": (
                integration_operation.branch_operation.result == BranchCreationResult.SUCCESS if integration_operation.branch_operation else False
            ),
    ...
    def report_validation_error(
        ...
        details = {"validation_error": validation_error, "branch_name": branch_name}
    ...
    def get_task_feedback_summary(self, task_id: str) -> Dict[str, Any]:
        ...
        if not task_entries:
            return {"task_id": task_id, "has_feedback": False, "total_entries": 0}
