...
from src.core.managers.feedback_manager import ProcessingStage
            ...
            for tm_task in tm_tasks:
                try:
                    ...
                    task_item = QueuedTaskItem(
                        ...
                        metadata={"taskmaster_task": tm_task, "source": "taskmaster"},
                    )
    ...
    def _execute_single_task(self, task_item: QueuedTaskItem) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_taskmaster_task and self.taskmaster_callback:
                ...
                transition_to_progress = self.status_manager.transition_status(task_item.page_id, "Queued to run", "In progress")
            ...
            if not is_taskmaster_task:
                self.feedback_manager.add_status_transition_feedback(task_item.page_id, "Queued to run", "In progress", True)
            ...
            self.feedback_manager.add_feedback(task_item.page_id, ProcessingStage.COPYING, "Starting task file copy to TaskMaster")
            ...
            if is_taskmaster_task and self.taskmaster_callback:
                ...
                # Use traditional status manager
                transition_to_done = self.status_manager.transition_status(task_item.page_id, "In progress", "Done")
                final_transition_success = transition_to_done.result == "success"

                if not final_transition_success:
                    self.feedback_manager.add_status_transition_feedback(task_item.page_id, "In progress", "Done", False, transition_to_done.error)
            ...
            if not is_taskmaster_task:
                self.feedback_manager.add_status_transition_feedback(task_item.page_id, "In progress", "Done", True)
            ...
            for line in status_lines:
                if len(line) >= 3:
                    ...
                    if not any(excluded in file_path for excluded in [".bak", "backup", ".log", ".pyc", "__pycache__"]):
                        ...
                        if status_first in ["M", "A", "D", "R", "C", "U"] or status_second in [
                            "M",
                            "A",
                            "D",
                            "R",
