...
class CommitMessageGenerator:
    ...
    def _detect_commit_type(self, task_data: TaskCommitData) -> CommitType:
        ...
        analysis_text = " ".join(
            filter(
                None,
                [task_data.task_title, task_data.task_description, task_data.completion_summary],
            )
        ).lower()
    ...
    def _optimize_description(self, description: str, commit_type: CommitType, scope: Optional[str], ticket_id: str) -> str:
        """
    ...
    def _validate_commit_message(self, message: str) -> "CommitValidationResult":
        ...
        return CommitValidationResult(is_valid=len(errors) == 0, errors=errors, warnings=warnings)
    ...
    def generate_batch_messages(self, tasks_data: List[TaskCommitData], commit_type: Optional[CommitType] = None) -> List[Tuple[str, str]]:
        """
