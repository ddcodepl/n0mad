...
from src.utils.checkbox_utils import CheckboxParser, CheckboxUtilities
...
class TaskStatusValidationService:
    ...
    def __init__(self, notion_client: NotionClientWrapper, cache_ttl_minutes: int = 5, enabled: bool = True):
        """
    ...
    def validate_task_transition(self, page_id: str, from_status: str, to_status: str, ticket_id: Optional[str] = None) -> ValidationOperation:
        """
    ...
    def _validate_commit_checkbox(self, page_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            commit_checkboxes = self.checkbox_utilities.find_checkbox_properties(page_data, self.commit_checkbox_names)
    ...
    def _requires_validation(self, from_status: str, to_status: str) -> bool:
        ...
        for valid_from, valid_to in self.validation_required_transitions:
            if normalized_lower[0] == valid_from.lower() and normalized_lower[1] == valid_to.lower():
                return True
    ...
    def _cache_checkbox_state(self, page_id: str, checkbox_name: str, checkbox_value: bool, confidence: float = 1.0):
        """
    ...
    def _cleanup_expired_cache(self):
        ...
        now = datetime.now()
        expired_keys = [key for key, entry in self._checkbox_cache.items() if now - entry.timestamp > self.cache_ttl]
    ...
    def get_validation_history(self, page_id: Optional[str] = None, limit: int = 100) -> List[ValidationOperation]:
        """
