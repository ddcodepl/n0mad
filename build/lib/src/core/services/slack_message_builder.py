...
class SlackMessageBuilder:
    ...
    def build_task_status_change_message(
        self,
        ...
        return {"channel": channel, "text": text, "blocks": blocks, "priority": priority}

    def build_task_completion_message(
        self,
        data: TaskStatusChangeData,
        channel: str,
    ...
    def build_system_alert_message(
        self,
        title: str,
        message: str,
        channel: str,
    ...
    def _build_status_change_blocks(self, data: TaskStatusChangeData, priority: MessagePriority) -> List[Dict[str, Any]]:
        ...
        details_fields.append({"type": "mrkdwn", "text": f"*Status:* {data.from_status} → {data.to_status}"})

        if data.user_id:
            details_fields.append({"type": "mrkdwn", "text": f"*Updated by:* <@{data.user_id}>"})

        details_fields.append({"type": "mrkdwn", "text": f"*Time:* {data.timestamp.strftime('%Y-%m-%d %H:%M:%S')}"})

        if data.commit_hash:
            commit_short = data.commit_hash[:8] if len(data.commit_hash) > 8 else data.commit_hash
            details_fields.append({"type": "mrkdwn", "text": f"*Commit:* `{commit_short}`"})
        ...
        if data.validation_result:
            blocks.append(
                {
                    "type": "context",
                    "elements": [{"type": "mrkdwn", "text": f"Validation: {data.validation_result}"}],
                }
            )
    ...
    def _build_completion_blocks(self, data: TaskStatusChangeData, priority: MessagePriority) -> List[Dict[str, Any]]:
        ...
        if data.user_id:
            details_fields.append({"type": "mrkdwn", "text": f"*Completed by:* <@{data.user_id}>"})

        if data.commit_hash:
            commit_short = data.commit_hash[:8] if len(data.commit_hash) > 8 else data.commit_hash
            details_fields.append({"type": "mrkdwn", "text": f"*Final commit:* `{commit_short}`"})

        blocks.append({"type": "section", "fields": details_fields})

        return blocks

    def _build_failure_blocks(self, data: TaskStatusChangeData, error_details: Optional[str], priority: MessagePriority) -> List[Dict[str, Any]]:
        ...
        details_fields.append({"type": "mrkdwn", "text": f"*Failed:* {data.timestamp.strftime('%Y-%m-%d %H:%M:%S')}"})
    ...
    def _build_bulk_update_blocks(self, updates: List[TaskStatusChangeData], priority: MessagePriority) -> List[Dict[str, Any]]:
        """Build Slack blocks for bulk update messages."""
        blocks = []

        # Summary header
        count = len(updates)
        blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": f"📊 *{count} Tasks Updated*"}})
        ...
        blocks.append({"type": "section", "fields": summary_fields[:10]})  # Limit to first 10 transitions
        ...
        if len(updates) <= 5:
            ...
            for update in updates[:5]:
                ...
                blocks.append({"type": "context", "elements": [{"type": "mrkdwn", "text": task_text}]})

        return blocks

    def _build_system_alert_blocks(self, title: str, message: str, alert_type: str, priority: MessagePriority) -> List[Dict[str, Any]]:
